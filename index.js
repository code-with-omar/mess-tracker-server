...
async function run() {
  try {
    ...
    const userDepositCollection = client
      .db("messTrackerDB")
      .collection("deposit");
    ...
    const managerAllHistory = client
      .db("messTrackerDB")
      .collection("managerHistory");
    const userPreviousMonth = client
      .db("messTrackerDB")
    ...
    app.patch("/addMeals/:id", verifyToken, verifyAdmin, async (req, res) => {
      try {
        const id = req.params.id;
        const filter = { _id: new ObjectId(id) };

        // data coming from frontend (body)
        const updatedData = req.body;
        // example: { meal: "3", date: "02" }
        ...
        const result = await userMealCollection.updateOne(filter, updateDoc);

        if (result.matchedCount === 0) {
          return res.status(404).send({ message: "Meal not found" });
        }

        res.send({ message: "Meal updated successfully", result });
      } catch (error) {
        console.error(error);
    ...
    app.delete("/addMeals/:id", verifyToken, verifyAdmin, async (req, res) => {
      try {
        ...
        const result = await userMealCollection.deleteOne(filter);
